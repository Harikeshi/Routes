...
ProryvK::ProryvK(const Input& input)
    : input{input}
{
}

Outputs::Route ProryvK::calculate()
{
    ...
    auto t_pervich_petlya = 1.3 * D_gas / Ut;
    ; // время , пока лодка находится в зоне 1 этапа поиска.

    bool if2ndSearchPossible = (Ut < Ukph); // ? true : false;
    ...
    //wpList.emplace_back(wp{Tbek_0, Ukph});
    ...
    Vec v03{Tbek_0, MT1.at(2)}; // от Tbek_0 до т.3 малого квадрата
    Vec v07{Tbek_0, MT1.at(6)}; // от Tbek_0 до т.7 малого квадрата

    double dis03 = v03.Abs(); // расстояние от Tbek_0 до т.3 малого квадрата
    double dis07 = v07.Abs(); // расстояние от Tbek_0 до т.7 малого квадрата
    ...
    double s_Perv = (dT_1st > 0 ? dT_1st : 0) * Ukp; // Put' po malomu kvadratu za t == dT_1st; if t < 0 , put' == 0
    ...
    double n_Perv_rem = n_Perv - n_Perv_i;           // Остаток ( ещё немного пройти, вдоль стороны малого квадрата)
    if (n_Perv_rem < 0)
    ...
    //    std::cout<< "--- indFirstCycle    = " << indFirstCycle << "\n" ;
    ...
    auto nextI = [&]() {
        ...
        // std::cout<< "\t\tnext_Ind_end = "<< indFirstCycle<<"\n";
        // static int raz=0;
        // std::cout<< "\t\textI_raz: "<< raz++<< "\n";
    };
    ...
    //if(prin)
    //{
    //    //<< "----- ind_quad_BeforeLast = "<< indBeforeLast<<"\n";
    ...
    if (if2ndSearchPossible) //Если будем во вторичный поиск.
    {
        ...
        int ind_mT2_0 = SegmLastTo10.Abs() < SegmLastTo16.Abs() ? 9 : 15; //chose MT1_10 : chose MT1_16
    ...
    */
    ...
    // for( k = index1stBegin+1; k<=index1stEnd; ++k )
    // {
    //      // суммируем угловые расстояния между wp пройденные в первичном поиске, видимые из Тпк:


    //     CBK_obsl += std::abs(
    //         std::abs( Vec{Tpk, wpList.at(k-1).pnt}.getCourse())
    //         -
    //         std::abs( Vec{Tpk, wpList.at( k ).pnt}.getCourse() )
    //           );
    // }
    ...
    //if(1)std::cout<<"~~~~~/endof__Proryv_k__~~~~~~~~~~\n\n";
