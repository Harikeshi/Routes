...
namespace Scene {
namespace Objects {
class RandomTargetObject : public QObject
{
public:
    RandomTargetObject(QObject* parent = nullptr)
        : QObject(parent), currentPosition{1e30, 1e30}
    {
    }
    ...
    QPointF generateRandomPoint(const PerimeterObject& perimeter, const Entities::Limits& limits)
    {
        ...
        while (!isValidPoint)
        {
            ...
            if (isPointInPolygon(randomPoint, perimeter.getRings()[0]))
            {
                isValidPoint = true;

                // Проверяем, находится ли точка внутри любого из внутренних многоугольников
                if (perimeter.getRings().size() > 1)
                {
                    for (size_t i = 1; i != perimeter.getRings().size(); ++i)
                    {
                        if (isPointInPolygon(randomPoint, perimeter.getRings()[i]))
                        {
                            isValidPoint = false;
                            break;
                        }
    ...
    // TODO: speed limits
    void movePoint(const PerimeterObject& perimeter, const Entities::Limits& limits, const double speedWithMultiplier)
    {
        ...
        QVector2D direction{static_cast<float>(next.x() - currentPosition.x()), static_cast<float>(next.y() - currentPosition.y())};
        direction.normalize();

        QVector2D step = direction * (speedWithMultiplier);
        QPointF newPoint = currentPosition + step.toPointF();

        if (isPointInPolygon(newPoint, perimeter.getRings()[0]))
        {
            ...
            if (perimeter.getRings().size() > 1)
                for (size_t i = 1; i != perimeter.getRings().size(); ++i)
                {
                    if (isPointInPolygon(newPoint, perimeter.getRings()[i]))
                    {
    ...
    void setStartPoint(const PerimeterObject& perimeter, const Entities::Limits& limits)
    {
        path.clear();
