...
/*!
 * Example:
 *  "INSERT INTO perimeters (id, entrance_x, entrance_y, exit_x, exit_y) "
            "VALUES ($1, $2, $3, $4, $5) "
            "ON CONFLICT (id) DO UPDATE SET "
            "entrance_x = $2, entrance_y = $3, exit_x = $4, exit_y = $5",
 */
    ...
    public:
        ...
        PostgreSQLRepository(const QString &connectionString)
                : Database::IRepository{connectionString},
                  m_connection(std::make_unique<pqxx::connection>(connectionString.toStdString())) {
        ...
        size_t save(const Route &route, pqxx::work &txn) {
            ...
            // Save route
            size_t route_id = txn.exec_params(
                    "INSERT INTO routes (id) VALUES (DEFAULT) RETURNING id")[0][0].as<size_t>();
