 ...
 * @return
 */
Path figure1P(QPointF& A, QPointF& B, const QPointF& C, double speed1, double speed2, double radius = radius1()) // TODO: (A,B,C,speed0,speed1)
{
    Path path;
    ...
    qDebug() << "Точка слева: " << *first_intersection << ", расстояние: " << geometry::distance(*first_intersection, AB_point);    //! Проверка
    qDebug() << "Точка справа: " << *second_intersection << ", расстояние: " << geometry::distance(*second_intersection, BC_point); //! Проверка
...
Path figure2L(QPointF& A, QPointF& B, const QPointF& C, double speed1, double speed2, double radius = radius1())
{
    ...
    std::cout << "Выходной сегмент: начало[" << seg3.start().x() << "," << seg3.start().y() << "], конец: [" << seg3.end().x() << "," << seg3.end().y() << "]" << std::endl;

    auto segs3 = seg3.upper(v, speed2, split_factor);
    v = segs3.back().speed();
    std::cout << "Скорость после отрезка 3: " << v << std::endl;

    std::cout << "Длина от начала дуги разворота до начала фигуры: " << geometry::distance(segs3.back().start(), arc1.start())
              << std::endl;
...
Path figure1A(QPointF& A, QPointF& B, const QPointF& C, double speed1, double speed2, double radius = radius1())
{
    ...
    qDebug() << "Направление биссектрисы: " << geometry::normalize(bisect) << "=" << geometry::angle(bisect) * RAD_TO_DEG;
